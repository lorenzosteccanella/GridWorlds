...
class KeyDoorTreasureGridWorld(gym.Env):
    metadata = {"render_modes": ["human", "rgb_array"], "render_fps": 800}

    def __init__(self, render_mode=None, max_steps=200, dense_reward=False, p=1.0):
        ...
        self.grid[6, :] = 1
        self.grid[:, 6] = 1
        self.grid[6, 3] = 0  # gate
        self.grid[3, 6] = 0  # gate
        self.grid[6, 9] = 0
        ...
        self._treasure_location = np.array([11, 11])
    ...
    def step(self, action):
        ...
        if np.array_equal(self._agent_location, self._treasure_location) and self.door_open:
            self.has_treasure = True
            self._treasure_location = np.array([-1, -1])  # Move treasure off-grid
            reward += 1

        terminated = self.has_treasure
    ...
    def _render_frame(self):
        ...
        if not self.has_treasure:
            pygame.draw.rect(
                canvas,
                (255, 255, 0),  # Yellow color for the treasure
                pygame.Rect(
                    pix_square_size * self._treasure_location,
                    (pix_square_size, pix_square_size),
                ),
            )
            text = font.render('T', True, (0, 0, 0))
            text_rect = text.get_rect(center=(pix_square_size * (self._treasure_location[0] + 0.5),
                                              pix_square_size * (self._treasure_location[1] + 0.5)))
            canvas.blit(text, text_rect)
...
if __name__ == "__main__":
    env = KeyDoorTreasureGridWorld(render_mode="human", dense_reward=True, p=1)
    env.human_play()